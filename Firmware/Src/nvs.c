...
{
    memcpy(nvs_memory, (uint32_t const*)&nvs_memory_flash[0], FLASH_CFG_SIZE);
    return NVS_OK;
}
...
nvs_result_t nvs_commit()
{
    ...
    erase_info.VoltageRange = FLASH_VOLTAGE_RANGE_3;
    erase_info.Sector = FLASH_CFG_START_SECTOR;
...
{
    memset(nvs_memory, 0, FLASH_CFG_SIZE);
    return NVS_OK;
}
...
nvs_result_t nvs_get(char const* key, uint8_t* dst, uint16_t* value_size, uint16_t dst_size)
{
    ...
    while (ptr < (nvs_memory + FLASH_CFG_SIZE - 12))
    {
        ...
        if (current_key == 0) {
            ...
            if (ptr + 12 + vs > nvs_memory + FLASH_CFG_SIZE) {
                break;
            }
...
nvs_result_t nvs_put(char const* key, uint8_t const* value, uint16_t value_size, uint16_t capacity_size)
{
    ...
    while (ptr < (nvs_memory + FLASH_CFG_SIZE - 12))
    {
        uint64_t const current_key = read64(ptr);
        if (current_key == 0) {
            if ((ptr + 12 + capacity_size) > (nvs_memory + FLASH_CFG_SIZE)) {
                break;
            }
            write64(ptr, k);
...
nvs_result_t nvs_del(char const* key)
{
    ...
    while (ptr < nvs_memory + FLASH_CFG_SIZE - 12)
    {
        ...
        if (current_key == 0) {
            ...
            // found
            uint16_t const entry_cap_size = read16(ptr + 8);
            if (ptr + 12 + entry_cap_size > nvs_memory + FLASH_CFG_SIZE) {
                break;
            }
            while (ptr + 12 + entry_cap_size < nvs_memory + FLASH_CFG_SIZE) {
                *ptr = *(ptr + 12 + entry_cap_size);
                ptr++;
            }
            while (ptr < nvs_memory + FLASH_CFG_SIZE) {
                *ptr = 0;
                ptr++;
            }
            return NVS_OK;
