...
static void MSC_Get_SerialNum(void);
static void MSC_IntToUnicode(uint32_t value, uint8_t * pbuf, uint8_t len);
...
__ALIGN_BEGIN uint8_t MSC_USBD_LangIDDesc[USB_LEN_LANGID_STR_DESC] __ALIGN_END =
{
...
__ALIGN_BEGIN uint8_t MSC_USBD_StrDesc[USBD_MAX_STR_DESC_SIZ] __ALIGN_END;

#if defined ( __ICCARM__ ) /*!< IAR Compiler */
  #pragma data_alignment=4   
#endif
__ALIGN_BEGIN uint8_t MSC_USBD_StringSerial[USB_SIZ_STRING_SERIAL] __ALIGN_END = {
...
{
  UNUSED(speed);
  *length = sizeof(MSC_USBD_LangIDDesc);
  return MSC_USBD_LangIDDesc;
}
...
uint8_t * USBD_FS_MSC_ProductStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_PRODUCT_STRING_FS, MSC_USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_PRODUCT_STRING_FS, MSC_USBD_StrDesc, length);
  }
  return MSC_USBD_StrDesc;
}
...
{
  UNUSED(speed);
  USBD_GetString((uint8_t *)USBD_MANUFACTURER_STRING, MSC_USBD_StrDesc, length);
  return MSC_USBD_StrDesc;
}
...
uint8_t * USBD_FS_MSC_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  *length = USB_SIZ_STRING_SERIAL;
  ...
  return (uint8_t *) MSC_USBD_StringSerial;
}
...
uint8_t * USBD_FS_MSC_ConfigStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  {
    USBD_GetString((uint8_t *)USBD_CONFIGURATION_STRING_FS, MSC_USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_CONFIGURATION_STRING_FS, MSC_USBD_StrDesc, length);
...
uint8_t * USBD_FS_MSC_InterfaceStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  if(speed == 0)
  {
    USBD_GetString((uint8_t *)USBD_INTERFACE_STRING_FS, MSC_USBD_StrDesc, length);
  }
  else
  {
    USBD_GetString((uint8_t *)USBD_INTERFACE_STRING_FS, MSC_USBD_StrDesc, length);
...
static void MSC_Get_SerialNum(void)
{
  ...
  if (deviceserial0 != 0)
  {
    MSC_IntToUnicode(deviceserial0, &MSC_USBD_StringSerial[2], 8);
    MSC_IntToUnicode(deviceserial1, &MSC_USBD_StringSerial[18], 4);
  }
}
